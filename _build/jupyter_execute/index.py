...
try:
    ...
    strain_L1, time_L1, chan_dict_L1 = rl.loaddata(data_path+fn_L1, 'L1')
except:
...
if make_plots:
    ...
    plt.savefig(figures_path+eventname+'_strain.'+plottype)
...
if make_plots:
    ...
    plt.title('Advanced LIGO strain data near '+eventname)
    plt.savefig(figures_path+eventname+'_ASDs.'+plottype)
...
if make_plots:
    ...
    plt.savefig(figures_path+eventname+'_H1_spectrogram.'+plottype)
    ...
    plt.savefig(figures_path+eventname+'_L1_spectrogram.'+plottype)
...
if make_plots:
    ...
    plt.title('aLIGO H1 strain data near '+eventname)
    plt.savefig(figures_path+eventname+'_H1_spectrogram_whitened.'+plottype)
    ...
    plt.axis([-0.5, 0.5, 0, 500])
    plt.title('aLIGO L1 strain data near '+eventname)
    plt.savefig(figures_path+eventname+'_L1_spectrogram_whitened.'+plottype)
...
try:
    f_template = h5py.File(data_path+fn_template, "r")
except:
...
if make_plots:
    ...
    plt.savefig(figures_path+eventname+'_template.'+plottype)
...
for det in dets:
    ...
    if make_plots:
        ...
        # -- Plot the result
        utils.plotter(time, timemax, SNR, pcolor, det, figures_path, 
...
# write the files:
utils.write_wavfile(audio_path+eventname+"_H1_whitenbp.wav",int(fs), strain_H1_whitenbp[indxd])
utils.write_wavfile(audio_path+eventname+"_L1_whitenbp.wav",int(fs), strain_L1_whitenbp[indxd])
...
utils.write_wavfile(audio_path+eventname+"_template_whiten.wav",int(fs), template_p_smooth[indxt])
...
fna = audio_path+eventname+"_template_whiten.wav"
print(fna)
Audio(fna)
...
fna = audio_path+eventname+"_H1_whitenbp.wav"
print(fna)
Audio(fna)
...
# write the files:
utils.write_wavfile(audio_path+eventname+"_H1_shifted.wav",int(fs), strain_H1_shifted[indxd])
utils.write_wavfile(audio_path+eventname+"_L1_shifted.wav",int(fs), strain_L1_shifted[indxd])

# and the template:
template_p_shifted = utils.reqshift(template_p_smooth,fshift=fshift,sample_rate=fs)
utils.write_wavfile(audio_path+eventname+"_template_shifted.wav",int(fs), template_p_shifted[indxt])
...
fna = audio_path+eventname+"_template_shifted.wav"
print(fna)
Audio(fna)
...
fna = audio_path+eventname+"_H1_shifted.wav"
print(fna)
Audio(fna)
...
if data_segments:
    ...
    strain, time, chan_dict = rl.loaddata(data_path+fn_L1, 'H1')
...
fmtcsv = ",".join(["%10.6f"] * 5)
np.savetxt(data_path+fncsv, datcsv, fmt=fmtcsv, header=headcsv)
